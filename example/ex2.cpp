...
GLTexture texture2("ground.ppm");

// 別クラスを作り、そこで球と立方体を動かす。
class Planet {
 public:
  explicit Planet(GLWorld *world) : world(world) {
    // 月の作成
...
int main() {
  ...
  camera.keepDepthMode(true);

  unsigned char imgbuf[450 * 150 * 4];
  float dptbuf[450 * 150];

  XWidget xwid;
  xwid.open(450, 150, "distance");
  xwid.setResizable(false);
  ...
  while (true) {
    ...
    //     if ( counter == 400 ) {
    //       camera.setDrawMode( GLDrawMode( DRAW_SOLID | WITH_TEXTURE ) );
    //     }

    usleep(10000);

    camera.getImage(imgbuf);
    camera.getDistance(dptbuf);
    //    camera.getDepth( dptbuf );

    GLWidget::convertDepthToImage(dptbuf, imgbuf, 450, 150);

    xwid.display(imgbuf, 32);
