...
Robot::Robot(const ColumnVector3 &length, const ColumnVector3 &shoulder_pos,
             const RotationMatrix &trank_rot)
    : length(length),
      Pos(0.0),
      initPos(shoulder_pos),
      initRot(trank_rot),
      angles(0.0) {
  //! 初期姿勢は全て単位行列.
...
void Robot::calcKinematics(const CColumnVector<7> &angles) {
  ...
  /////////////// 下腕
  ColumnVector3 lower(0.0);
  lower(2) = -length(1);

  ColumnVector3 axis_e(0.0);
  axis_e(1) = 1.0;
