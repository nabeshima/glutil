...
int main() {
  ...
  for (int i = 0; i < 9; ++i) {
    for (int j = 0; j < 9; ++j) {
      indices.push_back(
          IntVector3(j * 10 + i, (j + 1) * 10 + i, j * 10 + (i + 1)));
      indices.push_back(IntVector3((j + 1) * 10 + i, (j + 1) * 10 + (i + 1),
                                   j * 10 + (i + 1)));
    }
  }
  ...
  while (true) {
    ...
    double coeff = sin(static_cast<double>(counter) * 0.01);
    ...
    for (int i = 0; i < 10; ++i) {
      for (int j = 0; j < 10; ++j) {
        vert[j * 10 + i](2) =
            coeff * sqrt((i - 5.0) * (i - 5.0) + (j - 5.0) * (j - 5.0)) / 5.0;
      }
    }
