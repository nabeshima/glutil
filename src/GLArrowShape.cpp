...
void GLArrowShape::setParams(double trunk_radius, double trunk_length,
...
void GLArrowShape::drawSolid() const {
  ...
  double a = (M_PI * 2.0) / static_cast<double>(slices);
  ...
  // body
  {
    ...
    for (int i = 0; i <= slices; ++i) {
      glNormal3d(nx, ny, 0);
      glVertex3d(nx * trunk_radius, ny * trunk_radius,
                 trunk_length - head_length);
  ...
  // head
  double nz = head_radius / head_length;
  nx = 1.0;
  ny = 0.0;

  {
    ...
    for (int i = 0; i <= slices; ++i) {
      ...
      glNormal3d(nx, ny, nz);
      glVertex3d(nx * head_radius, ny * head_radius,
                 trunk_length - head_length);
