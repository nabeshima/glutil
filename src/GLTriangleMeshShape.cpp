...
GLTriangleMeshShape::GLTriangleMeshShape() 
  : smoothFlag( false ) {}
...
void GLTriangleMeshShape::setTriangles( const vector< ColumnVector3 > &vertices,
  ...
  for ( int i = 0; i < isize; ++i ) {
    const IntVector3 &index = indices[ i ];
    ColumnVector3
      d1( vertices[ index( 1 ) ] - vertices[ index( 0 ) ] ),
...
vector< ColumnVector3 >& GLTriangleMeshShape::getVertices() {
  return vertices;
}
...
void GLTriangleMeshShape::setSmooth( bool on ) {
  smoothFlag = on;
}
