...
void GLCameraViewer::mouseMoveEvent(int x, int y) {
  mousePressedButNotMoved = false;
  double diff_x = x - mouseX, diff_y = y - mouseY;

  mouseX = x;
  mouseY = y;

  ColumnVector3 eyePosTmp = this->getEyePosition(),
                eyeCenTmp = this->getEyeCenter(),
                eyeUpTmp = this->getEyeUpward();

  updateVisualAxis();

  if (mouseButton == 1 && shiftKeyPressed) {
    mouseButton = 2;
  }
  ...
  switch (mouseButton) {
    case 1:
      eyePosTmp +=
          -diff_x * L_MOUSE_GAIN * screenX + diff_y * L_MOUSE_GAIN * screenY;
    ...
    case 3:
      eyePosTmp +=
          R_MOUSE_GAIN * -diff_x * screenX + R_MOUSE_GAIN * diff_y * screenY;
      eyeCenTmp +=
          R_MOUSE_GAIN * -diff_x * screenX + R_MOUSE_GAIN * diff_y * screenY;
      this->setEye(eyePosTmp, eyeCenTmp, eyeUpTmp);
      repaint(false);
    ...
    case 2: {
      if ((diff_y < 0 && distanceEye > near) ||
        ...
        ColumnVector3 div2 =
            (eyeCenTmp - eyePosTmp) * MID_MOUSE_GAIN * diff_y / distanceEye;
        eyePosTmp -= div2;
      }
      this->setEye(eyePosTmp, eyeCenTmp, eyeUpTmp);
      repaint(false);
