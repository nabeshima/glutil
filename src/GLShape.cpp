...
void GLShape::draw(GLDrawMode drawMode) const {
  ...
  MaterialScope ms(getAmbient(), getDiffuse(), getSpecular(), getEmission(),
                   getShininess());
...
BeginEndScope::BeginEndScope(uint32_t mode) { glBegin(mode); }

BeginEndScope::~BeginEndScope() { glEnd(); }

EnableDisableScope::EnableDisableScope(uint32_t mode) : mode(mode) {
  glEnable(mode);
}

EnableDisableScope::~EnableDisableScope() { glDisable(mode); }

DisableEnableScope::DisableEnableScope(uint32_t mode) : mode(mode) {
  glDisable(mode);
}

DisableEnableScope::~DisableEnableScope() { glEnable(mode); }

CoordinateScope::CoordinateScope(cotave::Quaternion quaternion,
                                 cotave::ColumnVector3 position, double scale) {
  glPushMatrix();
...
CoordinateScope::~CoordinateScope() { glPopMatrix(); }

MaterialScope::MaterialScope(const GLColor &ambient, const GLColor &diffuse,
...
BlendScope::BlendScope(bool isBlend) : isBlend(isBlend) {
  if (isBlend) {
...
TextureShaderScope::TextureShaderScope(const GLTexture *texture,
  ...
  if (texture != NULL) {
    ...
    glBindTexture(
        GL_TEXTURE_2D,
        texture
            ->getResourceID());  // 0番のテクスチャユニットにリソースを割り当て
  }

  if (shader != NULL) {
    glUseProgram(shader->getResourceID());
    glUniform1i(glGetUniformLocation(shader->getResourceID(), "texture"),
                0);  // 0番のテクスチャユニットを渡す.
  } else if (texture != NULL) {
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE,
              GL_MODULATE);  //! シェーダのときは自前でやる
