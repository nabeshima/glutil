...
class GLWidget : public cutil::XWidget {
 private:
  //! 次の動作: -1ならinitializeGLを呼ぶ. 0なら何もしない.
  //! 1ならfinalizeGLを呼んでからinitializeGLを呼ぶ.
  enum NextOp { INIT, RESET, NONE };
  NextOp nextop;
  ...
  */
  ...
  //! if keepImageFlag is true, this function force the widget not to be
  //! resized.
  ...
  */
  ...
  static void convertDepthToImage(const float *depthbuf, unsigned char *buf32,
