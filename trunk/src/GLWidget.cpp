...
XVisualInfo GLWidget::chooseVisual() {
  XVisualInfo *vi = glXChooseVisual( cutil::XEventThread::getDisplay(),
                                     cutil::XEventThread::getScreen(),
                                     attr_stencil );
  return vi[ 0 ];
}
...
void GLWidget::operator()() {
  ...
  while ( true ) {
	...
	glReadPixels( 0, 0, getWidth(), getHeight(), GL_RGBA, GL_UNSIGNED_BYTE, imagebuf );
      }
      
      if ( keepDepthFlag ) {
	glReadPixels( 0, 0, getWidth(), getHeight(), GL_DEPTH_COMPONENT, GL_FLOAT, depthbuf );
      }
    }
...
void GLWidget::keepImageMode( bool keepImageFlag ) {
  ...
  if ( !this->keepImageFlag && keepImageFlag ) { //! 一度trueにしたら,バッファが作られる.
  ...
  if ( keepImageFlag ) {
    setResizable( false );    //! resizing causes crashes of glReadPixels, getImage and so on.
  }
...
bool GLWidget::writePPM( const string &filename ) {
  ...
  char upside_down_image[ image_size ];
  
  {
	...
	memcpy( (void*)( upside_down_image + off2 ),  (void*)( imagebuf + off ), 3 );
	off += 4;
	off2 += 3;
      }
    }
  ...
  return PPMFileIO::write( filename, upside_down_image, width, height );
}
...
void GLWidget::keepDepthMode( bool keepDepthFlag ) {
  ...
  if ( keepDepthFlag ) {
    setResizable( false );    //! resizing causes crashes of glReadPixels, getDepth and so on.
  }

  this->keepDepthFlag = keepDepthFlag;
}

// depth値が1.0fだと,何も描画されていない領域. 値の符号を負に変えて, フラグにする.
...
void GLWidget::convertDepthToImage( const float *depthbuf, unsigned char *buf32, int width, int height ) {
  ...
  for ( int j = 0; j < height; ++j ) {
    for ( int i = 0; i < width; ++i ) {
      ...
      unsigned char gry = ( val > 0 )? ( ( f32[ ( height - j - 1 ) * width + i ] - min ) / ( max - min ) * 255.0 ): 0;
