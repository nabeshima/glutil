...
void GLCamera::open( const GLWorld *world, int width, int height, const std::string &title ) {
  this->world = world;
...
void GLCamera::initializeGL() {
  ...
  glLoadIdentity();
  gluPerspective( fovy, (double)getWidth() / (double)getHeight(), near, far );
...
void GLCamera::paintGL() {
  ...
  
  {
    ...
    gluLookAt( eyePos( 0 ), eyePos( 1 ), eyePos( 2 ), 
	       ...
	       eyeUpward( 0 ), eyeUpward( 1 ), eyeUpward( 2 ) );
  }
...
void GLCamera::displayText() {
  ...
  if ( !textList.empty() ) {
    ...
    std::list< const GL2DText* >::iterator
      it = textList.begin(),
      endIt = textList.end();
...
ColumnVector3 GLCamera::getEyePosition() const {
  return eyePos; 
}

ColumnVector3 GLCamera::getEyeCenter() const { 
  return eyeCenter;
}

ColumnVector3 GLCamera::getEyeUpward() const { 
  return eyeUpward;
}
