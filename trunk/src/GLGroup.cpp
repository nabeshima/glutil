...
void GLGroup::draw( GLDrawMode drawMode ) const {
  CoordinateScope cds( getQuaternion(), getPosition(), getScale() );  //! 回転や変位
  ...
  if ( isStatic ) {
    ...
    */
    ...
    if ( group != NULL ) {
      ...
      childList.insert( childList.begin(),
                        group->childList.begin(),
                        group->childList.end() );
