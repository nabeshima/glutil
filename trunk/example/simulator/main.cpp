...
int main( int argc, char **argv ){
  ...
  length( 0 ) = 0.30; length( 1 ) = 0.20; length( 2 ) = 0.10;
  trank_rot( 0, 0 ) = 1.0; trank_rot( 1, 1 ) = 1.0; trank_rot( 2, 2 ) = 1.0;
  shoulder_pos( 1 ) = 0.13; shoulder_pos( 2 ) = 0.15;
  ...
  //  plane->setQuaternion( Quaternion( ColumnVector3( 0, 1, 0 ), - M_PI / 4.0 ).normalize() );
  ...
  mirror->setQuaternion( Quaternion( ColumnVector3( 0, 1, 0 ), - M_PI / 4.0 ).normalize() );
  //  mirror->setQuaternion( Quaternion( ColumnVector3( 0, 1, 0 ), - M_PI / 2.0 ).normalize() );
  ...
  epos[ 0 ] = -3.99999; epos[ 1 ] = 2.9405; epos[ 2 ] = 1.08698;
  ecen[ 0 ] = 0.60157; ecen[ 1 ] = 0.104408; ecen[ 2 ] = 0.157535;
  eup[ 0 ] = 0; eup[ 1 ] = 0; eup[ 2 ] = 1;
  ...
  double 
    amp = M_PI * 2.0 / 255.0 * 41.0,
    off = M_PI * 2.0 / 255.0 * 41.0;
  ...
  while( true ){
    ...
    for( int i = 0; i < 7; ++i ){ angles( i ) = data[ i ]; }

    //! ダミーの物体を動かす.
    box->setPosition( ColumnVector3( 0.3 * sin( time ) + 0.2, 0.3 * cos( time ) + 0.3, 0.005 * sin( 2.0 * time ) + 0.6 ) );
